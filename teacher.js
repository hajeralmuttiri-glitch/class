import { TEAMS, STAGES, STAGE_LABELS, STAGE_DURATIONS, VOTING_CRITERIA, TEAM_QUESTIONS } from "./data.js";
...
function initDashboard() {
  ...
  onValue(ref(db, "session/competitionStartTime"), (snap) => {
    ...
    sessionTimerInterval = setInterval(() => {
      ...
      if (remaining <= 0) { clearInterval(sessionTimerInterval); resetSession(); return; }
      const m = Math.floor(remaining / 60000), s = Math.floor((remaining % 60000) / 1000);
  ...
  document.getElementById("resetBtn").onclick = async () => {
    if (!confirm("هل أنتِ متأكدة من تصفير كل البيانات (الإجابات، النقاط، التصويت) لبدء فصل جديد؟")) return;
    await resetSession();
  };
}
